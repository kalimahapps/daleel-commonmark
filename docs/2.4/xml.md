...
Version 2.0 introduced the ability to render Markdown `Document` objects in XML. This is particularly useful for debugging [custom extensions](/customization/overview) as you can see the XML representation of the [Abstract Syntax Tree](/customization/abstract-syntax-tree).

To convert Markdown to XML, you would instantiate a `MarkdownToXmlConverter` with an [`Environment`](/customization/environment) and then call `convert()` on any Markdown.
...
See the [rendering documentation](/customization/rendering#xml-rendering) for information on customizing the XML output.
