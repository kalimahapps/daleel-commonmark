...
Alternatively, if you don't want all of the core Markdown syntax, avoid using `CommonMarkCoreExtension`.  You can always add just the individual parsers, renderers, etc. you actually want with the [`Environment`](/customization/environment).  (This is actually how the [Inlines Only Extension](/extensions/inlines-only) works - it only includes a subset of things that `CommonMarkCoreExtension` does!)
