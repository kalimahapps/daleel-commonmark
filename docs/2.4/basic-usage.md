...
:::warning
 See the [security](/security) section for important details on avoiding security misconfigurations.
:::
...
The `CommonMarkConverter` and `GithubFlavoredMarkdownConverter` shown above automatically configure [the environment](/customization/environment) for you, but if you want to use [additional extensions](/customization/extensions) you'll need to avoid those classes and use the generic `MarkdownConverter` class instead to customize [the environment](/customization/environment) with whatever extensions you wish to use:
...
See the [configuration section](/configuration) for more information on the available configuration options.
